...
public partial class Player : Entity
{
    ...
    [Command]
    void CmdNavigateDestination(Vector2 position, float stoppingDistance)
    {
        navigatePosition = position; navigateStop = stoppingDistance;
        cmdEvents.Add("NavigateDestination");
    }
    bool EventNavigateDestination()
    { return cmdEvents.Remove("NavigateDestination"); }

    Vector2 navigateVelocity = Vector2.zero;
    [Command]
    void CmdNavigateVelocity(Vector2 velocity)
    {
    ...
    string UpdateServer_MOVING()
    {
        ...
        if (navigateVelocity != Vector2.zero)
            agent.velocity = navigateVelocity * agent.speed;
    ...
    void SetIndicatorViaPosition(Vector2 pos)
    {
        if (!indicator) indicator = Instantiate(indicatorPrefab);
